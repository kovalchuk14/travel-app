...
export default function StoriesPageClient({
  ...
  setStories(safeStories.slice(0, STORIES_PER_PAGE)); setPage(1); setCanLoadMore(safeStories.length >= STORIES_PER_PAGE);
  ...
  const updateCategory = async (newCategory: string) => {
    ...
    try {
      ...
      console.error('Виникла помилка при отриманні історій за категорією:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleLoadMore = async () => {
    ...
    try {
      ...
      const newStories = await fetchStories(nextPage, STORIES_PER_PAGE, categoryParam);
